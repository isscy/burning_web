...
import {igetRequst,getRequst,postRequst,putRequst, deleteRequst} from "@/api/data";
...
export default {
    ...
    methods: {
        loadData(){
            ...
            getRequst('contactUs/list/admin', param, getToken()).then(res => {
                let r = res.data;
        ...
        delete(id){
            deleteRequst('contactUs/' + id , {}).then(res => {
                let r = res.data;
                if(r.code != '1'){
                    throw r.message;
                }
                ...
                this.loading = false;
                this.$Message.error('删除错误! ' + err); return false;
            })
    ...
    created() {
        this.tableTitle = [
            ...
            //             text = '未读';sty = {color:'bule'}
            //         }
            //         if(params.row.status == '1'){
            //             text = '已读';sty = {color:'red'}
            //         }
                            ...
                            on: {
                                click: () => {
                                    this.$Modal.confirm({
                                        ...
                                        onOk: () => { this.delete(params.row.id)},
                                    });
                                }
                            }
                        }, '删除'),
